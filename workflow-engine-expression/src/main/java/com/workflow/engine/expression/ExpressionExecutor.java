...
import com.workflow.engine.expression.format.ExpressionParser;
import com.workflow.engine.expression.format.FormatException;
import com.workflow.engine.expression.function.FunctionExecution;
import com.workflow.engine.expression.op.ConstantEvaluator;
...
/**
 * IK-Expression表达式执行器, 所谓逆波兰式即后缀表达式
 *
 * @author 林良益，卓诗垚
 * @version 2.0
 *          2008-09-18
...
public class ExpressionExecutor {
    ...
    public List<ExpressionToken> compile(List<ExpressionToken> expTokens) throws IllegalExpressionException {
        ...
        for (ExpressionToken expToken : expTokens) {
            ...
            if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()) {
                ...
                if (opStack.empty()) {//如果操作栈为空
                    ...
                    while (!opStack.empty() && doPeek) {
                        ...
                        } else if (ExpressionToken.ETokenType.ETOKEN_TYPE_SPLITTER == onTopOp.getTokenType()
            ...
            } else if (ExpressionToken.ETokenType.ETOKEN_TYPE_CLASS == expToken.getTokenType()) {
                _class = expToken;
                ...
                if (_class != null) {
                    ...
                    opStack.push(_class);
                    //压入校验栈
                    verifyStack.push(_class);
                    //清空临时类型变量
                    _class = null;
                }

            } else if (ExpressionToken.ETokenType.ETOKEN_TYPE_SPLITTER == expToken.getTokenType()) {
                //处理读入的“（”
                if ("(".equals(expToken.getSplitter())) {
                    ...
                    while (doPop && !opStack.empty()) {
                        ...
                        if (ExpressionToken.ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()) {
                            ...
                            // 如果遇到"(", 则操作结束
                            doPop = false;
                            //查看栈顶元素是否是ETOKEN_TYPE_CLASS, 若是则加入RPN队列
                            if (!opStack.empty() && ExpressionToken.ETokenType.ETOKEN_TYPE_CLASS == opStack.peek().getTokenType()) {
                                onTopOp = opStack.pop();
                                _RPNExpList.add(onTopOp);
                            }
                        }
    ...
    public Constant execute(List<ExpressionToken> _RPNExpList) throws IllegalExpressionException {
        ...
        for (ExpressionToken expToken : _RPNExpList) {
            ...
            if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()) {
                ...
                ((Reference)onTop.getConstant().getDataValue()).setDataType(BaseDataMeta.TYPE_MAPPING.get(expToken.getCastType()));
                //转换类型后的token压入编译栈
                compileStack.push(onTop);
    ...
    public String tokensToString(List<ExpressionToken> tokenList) {
        ...
        for (ExpressionToken token : tokenList) {
            ...
            } else if (ExpressionToken.ETokenType.ETOKEN_TYPE_CLASS == tokenType) {
            ...
            } else if (ExpressionToken.ETokenType.ETOKEN_TYPE_SPLITTER == tokenType) {
                expressionText.append(token.getSplitter()).append(" ");

            }
        }
        expressionText.delete(expressionText.length() - 1, expressionText.length());
        return expressionText.toString();
    }

    /**
     * 将表达式子串（格式化好的），转换成词元列表
     *
    ...
    private void addToken(String tokenString, List<ExpressionToken> tokens) throws IllegalExpressionException {
        ...
        if (ExpressionTokenHelper.isNull(tokenString)) { //null
            ...
            tokens.add(token);
        } else if (ExpressionTokenHelper.isSplitter(tokenString)) { //分割符
        ...
        if (!verifyStack.empty()) {
            ...
            while (doPop && !verifyStack.empty()) {
                ...
                if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == parameter.getTokenType()) {
                    ...
                    //没有找到应该存在的右括号
                    throw new IllegalExpressionException("表达式不合法，函数\"" + functionToken.getFunctionName() + "\"遇到非法参数" + parameter.toString() + ";位置:" + parameter.getStartPosition()
                            , functionToken.toString()
                            , functionToken.getStartPosition());
                }
            }

            ExpressionToken castTypeToken = null;
            if (!verifyStack.empty()){
                ExpressionToken onTop = verifyStack.peek();
                //若校验栈中仍有元素,栈顶元素可能是强转类型词元,也可能不是,这里要做判断,否则在函数嵌套调用的时候也按照强转类型的逻辑执行,导致出错
                if(onTop.getTokenType().equals(ExpressionToken.ETokenType.ETOKEN_TYPE_CLASS)){
                    castTypeToken = verifyStack.pop();
                }
            }

            if (doPop && verifyStack.empty()) {
                //操作栈已空，没有找到函数的左括号（
                throw new IllegalExpressionException("表达式不合法，函数\"" + functionToken.getFunctionName() + "\"缺少\"(\"；位置:" + (functionToken.getStartPosition() + functionToken.toString().length())
                        , functionToken.toString()
                        , functionToken.getStartPosition());
            }
            ...
            Constant result = FunctionExecution.verify(functionToken.getFunctionName(), functionToken.getStartPosition(), arguments, castTypeToken);
            return ExpressionToken.createConstantToken(result);


        } else {
            //没有找到应该存在的右括号
            throw new IllegalExpressionException("表达式不合法，函数\"" + functionToken.getFunctionName() + "\"不完整"
                    , functionToken.toString()
                    , functionToken.getStartPosition());
        }
    }
