 ...
 */
@Deprecated
public class CreateVehicleInfo implements IStep {

    private static final Logger logger = LoggerFactory.getLogger(CreateVehicleInfo.class);
    ...
    private static Map<String, Object> doGetVehicleInfo(Map<String, String> conditions) throws Exception {
        ...
        if (queryString.size() == 3) {
            ...
            if (getIntegerNodeByKey(jsonNode, "code") != 0) {
                logger.info("按照条件未查询到车型列表");
                vehicleInfo.put("code", "1");
                return vehicleInfo;
            }
            //第一个请求查询是默认会添加brand_name
            queryString.put("brand_name", getMapNodeByKey(jsonNode, "selected").get("brand_name"));
        }
        ...
        //车型列表
        vehicleInfo.put("vehicles", getListNodeByKey(jsonNode, "vehicles"));
        //车型筛选条件
        vehicleInfo.put("distincts", getMapNodeByKey(jsonNode, "distincts"));
        //车型分页信息
        vehicleInfo.put("pagination", getMapNodeByKey(jsonNode, "pagination"));
        return vehicleInfo;
    }
    ...
    private static JsonNode sendFirstRequest(Map<String, String> queryString) throws Exception {
        return getJsonNode(executeGetMethod(_FIRST_REQUEST_URL, queryString));
    }
    ...
    private static JsonNode sendSecondRequest(Map<String, String> queryString) throws Exception {
        return getJsonNode(executeGetMethod(_SECOND_REQUEST_URL, queryString));
    }
